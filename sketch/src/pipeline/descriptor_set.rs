...
impl DescriptorSet {
    pub fn new(device: &Device, descriptor_layout: vk::DescriptorSetLayout, image_count: u32, descriptor_pool: &DescriptorPool, ubos: &Vec<UniformBufferObject>) -> DescriptorSet {
        ...
        descriptor_sets.iter().zip(ubos.iter()).for_each(|(set, buffer)| {
            let buffer_info = vk::DescriptorBufferInfo::builder().buffer(*buffer.vulkan_object()).offset(0).range(UniformTestObject::get_size() as vk::DeviceSize).build();
